...
def get_source_directory():
    ...
    parser = argparse.ArgumentParser(
        description="Convert ledger prices to beancount prices"
    )
...
def main():
    ...
    for filename in os.listdir(source_directory):
        ...
        destination = os.path.join(
            source_directory, filename.replace(".ledger", ".bean")
        )
        ...
        with open(source, "r", encoding="utf-8") as src:
            ...
            with open(destination, "w", encoding="utf-8") as dest:
                for line in src:
                    ...
                    if price:
                        # Beancount price does not have time component.
                        dest.write(
                            f"{price['date']} price {price['commodity']} {price['amount']} {price['currency']}\n"
                        )
                    else:
